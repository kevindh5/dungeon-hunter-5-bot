...
bot.on("message", (msg) => {
    ...
    if (msg.member === null) { //evite un bug quand le bot envoie des messages privés
        ...
        const role = msg.member.roles.find(role => {
            return role.name === ROLE
        }); //on voit si il al erole leader
        if (role !== null && estCommandLead(msg.content) !== false) { //on regarde si il a le role leader
            ...
            switch (command) {
                case "!newGuerre":
                    if (lastWarMsg[msg.guild.name].length > 0) { //on renitialise les LastWarMsg pour que la func delGuerre marche bien.
                    ...
                    break;
                case "!delGuerre":
                    guerre.delGuerre(msg, lastWarMsg);
                    break;
                case "!rebootGuerre":
                    guerre.rebootGuerre(msg, lastWarMsg);
                    break;
                default:
...
bot.on("guildMemberUpdate", (before, after) => {
    const ancienRoles = before.roles.find(role => {
        return role.name === ROLE
    }); // on voit si il avait le role leader 
    const nouveauRoles = after.roles.find(role => {
        return role.name === ROLE
    });
